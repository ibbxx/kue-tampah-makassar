    ...
    <div className="flex min-h-screen items-center justify-center bg-gradient-to-br from-secondary via-background to-secondary/30 px-4">
      <div className="w-full max-w-md rounded-2xl border border-border bg-card p-8 shadow-xl">
        ...
        <form onSubmit={submit} className="mt-8 space-y-4">
          ...
          <button
            disabled={busy}
            className="inline-flex w-full items-center justify-center gap-2 rounded-full bg-primary py-3 font-semibold text-primary-foreground hover:opacity-90 disabled:opacity-50"
          >
            <Lock className="h-4 w-4" /> {busy ? "Memproses..." : "Login"}
          </button>
        </form>
        <p className="mt-4 text-center text-xs text-muted-foreground">
          Belum punya akun admin? Buat lewat dashboard Supabase, lalu tambahkan role 'admin' di
          tabel <code>user_roles</code>.
        </p>
