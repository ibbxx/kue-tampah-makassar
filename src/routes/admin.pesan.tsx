  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground">Inbox dari form kontak pelanggan.</p>
      </div>
      <div className="space-y-3">
        {(data ?? []).map((m) => (
          <div
            key={m.id}
            className={`rounded-2xl border p-5 ${m.is_read ? "border-border bg-card" : "border-primary/40 bg-primary/5"}`}
          >
